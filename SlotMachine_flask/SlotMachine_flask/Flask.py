...
def index():
    ...
    return render_template("index.html", saldo=game.saldo, bet=game.get_bet(), last_results=last_results, last_win=last_win)

@app.route("/spin")
@login_required
def spin():
    game = get_game()
    if game.saldo < game.get_bet():
        flash("Insufficient funds", "information")
        return redirect(url_for("index"))
    result = game.spin()
    save_game(game)
    session["last_results"] = result["results"]
    session["last_win"] = result["win"]
...
def raise_bet():
    game = get_game()
    if game.bet_index == len(game.bets) - 1:
        flash("Already at maximum bet", "information")
        return redirect(url_for("index"))
    game.raise_bet()
    save_game(game)
    return redirect(url_for("index"))
...
def lower_bet():
    game = get_game()
    if game.bet_index == 0:
        flash("Already at minimum bet", "information")
        return redirect(url_for("index"))
    game.lower_bet()
    save_game(game)
    return redirect(url_for("index"))
